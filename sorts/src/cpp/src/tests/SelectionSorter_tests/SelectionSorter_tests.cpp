...
TEST(random, selection_sorter_test_suite) {
    ...
    auto sorter = kogan::make_unique<kogan::SelectionSorter<int>>([](int a, int b) { return a - b; }, sequence);
    sorter->sort();
...
kogan::TestSuite get_selection_sorter_test_suite() { return selection_sorter_test_suite; }