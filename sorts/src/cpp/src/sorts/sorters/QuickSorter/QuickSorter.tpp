...
template <class T>
inline void QuickSorter<T>::sort_() {
...
template <class T>
inline bool QuickSorter<T>::partition_() {
    if (!is_partitioning_) {
...
inline SmartPtrLinkedListSequence<std::size_t> QuickSorter<T>::generate_interesting_indexes_() const noexcept {
    ...
    if (i_ >= 0 && i_ < n) interesting_indexes.append(i_);
    if (j_ >= 0 && j_ < n) interesting_indexes.append(j_);
