...
template <class T> inline UniquePtr<T>::operator bool() const noexcept {
  return get() != nullptr;
}
...
template <class T> inline T *UniquePtr<T[]>::get() const noexcept {
  return ptr_;
}

template <class T> inline T *UniquePtr<T>::operator->() const noexcept {
  return ptr_;
}
