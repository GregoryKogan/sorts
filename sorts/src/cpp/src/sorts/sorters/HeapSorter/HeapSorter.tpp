...
template <class T>
inline void HeapSorter<T>::sort_() {
    if (!started_) {
...
template <class T>
inline void Heapifier<T>::heapify() {
    if (!started_) {
        largest_ = index_;
        l_ = 2 * index_ + 1;
        r_ = 2 * index_ + 2;
    ...
    if (!left_checked_) {
        if (l_ < heap_size_) {
            std::optional<int> cmp =
                sorter_->cmp_wrapper_(sorter_->sequence_->get(l_), sorter_->sequence_->get(largest_));
            if (!cmp) return;
            if (cmp.value() > 0) largest_ = l_;
        }
    ...
    if (!right_checked_) {
        if (r_ < heap_size_) {
            std::optional<int> cmp =
                sorter_->cmp_wrapper_(sorter_->sequence_->get(r_), sorter_->sequence_->get(largest_));
            if (!cmp) return;
            if (cmp.value() > 0) largest_ = r_;
        }
    ...
    if (largest_ != index_) {
        if (!largest_swap_done_) {
            if (!sorter_->swap_(index_, largest_)) return;
...
inline SmartPtrLinkedListSequence<std::size_t> Heapifier<T>::generate_interesting_indexes() const noexcept {
    ...
    if (0 <= index_ && index_ < sorter_->sequence_->get_length()) interesting_indexes.append(index_);
...
inline void HeapSorter<T>::set_interesting_indexes_() noexcept {
    ...
    if (!extracting_started_) {
        if (0 <= i_ && i_ < this->sequence_->get_length()) this->interesting_indexes_->append(i_);
    } else {
        if (0 <= j_ && j_ < this->sequence_->get_length()) this->interesting_indexes_->append(j_);
    }
}
