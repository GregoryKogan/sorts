...
template <class T> inline SmartPtrDynamicArray<T>::SmartPtrDynamicArray() {
  length_ = 0;
  data_ = SharedPtr<T[]>();
}

template <class T> inline SmartPtrDynamicArray<T>::SmartPtrDynamicArray(int length) {
  if (length < 0) throw InvalidArgumentException("length must be positive");
  length_ = length;
  data_ = make_shared<T[]>(length);
}

template <class T> inline SmartPtrDynamicArray<T>::SmartPtrDynamicArray(SharedPtr<T[]> data, int length) {
  if (length < 0) throw InvalidArgumentException("length must be positive");
...
template <class T> inline SmartPtrDynamicArray<T>::SmartPtrDynamicArray(const SmartPtrDynamicArray<T> &other) {
...
template <class T> inline std::size_t SmartPtrDynamicArray<T>::get_length() const noexcept { return length_; }

template <class T> inline void SmartPtrDynamicArray<T>::set(int index, T value) {
  if (index < 0 || index >= length_) throw IndexOutOfRangeException(index, 0, length_ - 1);
  data_[index] = value;
}
...
template <class T> inline T &SmartPtrDynamicArray<T>::operator[](int index) {
  if (index < 0 || index >= length_) throw IndexOutOfRangeException(index, 0, length_ - 1);
