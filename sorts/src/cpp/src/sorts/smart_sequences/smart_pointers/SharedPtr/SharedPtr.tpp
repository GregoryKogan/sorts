...
template <class T> inline void SharedPtr<T>::update_reference(ControlBlock<T> *new_control_block) {
...
template <class T> inline SharedPtr<T>::SharedPtr() noexcept { // default constructor
  control_block_ = nullptr;
}

template <class T> inline SharedPtr<T[]>::SharedPtr() noexcept { // default constructor for arrays
  control_block_ = nullptr;
}

template <class T> inline SharedPtr<T>::SharedPtr(T *ptr) noexcept { // constructor from pointer
  control_block_ = new ControlBlock<T>(ptr);
}

template <class T> inline SharedPtr<T[]>::SharedPtr(T *ptr) noexcept { // constructor from pointer for arrays
  control_block_ = new ControlBlock<T[]>(ptr);
}
...
template <class T> inline SharedPtr<T>::SharedPtr(const WeakPtr<T> &other) { // constructor from WeakPtr
  if (other.expired()) throw EmptyPointerException();
  control_block_ = other.control_block_;
  if (control_block_ && control_block_->get() != nullptr) control_block_->increment_reference_counter();
}

template <class T> inline SharedPtr<T[]>::SharedPtr(const WeakPtr<T[]> &other) { // constructor from WeakPtr for arrays
  if (other.expired()) throw EmptyPointerException();
  control_block_ = other.control_block_;
  if (control_block_ && control_block_->get() != nullptr) control_block_->increment_reference_counter();
}
...
template <class T> inline SharedPtr<T>::~SharedPtr() { update_reference(nullptr); }

template <class T> inline SharedPtr<T[]>::~SharedPtr() { update_reference(nullptr); }

template <class T> inline SharedPtr<T> &SharedPtr<T>::operator=(const SharedPtr<T> &other) noexcept { // copy assignment
  if (this != &other) {
...
template <class T> inline SharedPtr<T> &SharedPtr<T>::operator=(SharedPtr<T> &&other) noexcept { // move assignment
  if (this != &other) {
    update_reference(other.control_block_);
    other.control_block_ = nullptr;
  }
...
template <class T> inline SharedPtr<T> &SharedPtr<T>::operator=(std::nullptr_t) noexcept { // assignment from nullptr
...
template <class T> inline void SharedPtr<T>::swap(SharedPtr<T> &other) noexcept {
...
template <class T> inline bool SharedPtr<T>::unique() const noexcept { return use_count() == 1; }

template <class T> inline bool SharedPtr<T[]>::unique() const noexcept { return use_count() == 1; }

template <class T> inline SharedPtr<T>::operator bool() const noexcept { return get() != nullptr; }
...
template <class T> inline T *SharedPtr<T>::operator->() const noexcept { return get(); }

template <class T> inline T &SharedPtr<T>::operator*() const noexcept { return *(get()); }

template <class T> inline T &SharedPtr<T[]>::operator[](std::size_t index) const { return get()[index]; }

template <class T> inline unsigned int SharedPtr<T>::use_count() const noexcept {
