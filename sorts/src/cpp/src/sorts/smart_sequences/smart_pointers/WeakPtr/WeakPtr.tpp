...
inline void WeakPtr<T>::update_reference(ControlBlock<T> *new_control_block) {
  if (control_block_)
    control_block_->decrement_weak_ptr_reference_counter_and_delete_if_zero();
  control_block_ = new_control_block;
}

template <class T>
inline void
WeakPtr<T[]>::update_reference(ControlBlock<T[]> *new_control_block) {
  if (control_block_)
    control_block_->decrement_weak_ptr_reference_counter_and_delete_if_zero();
  control_block_ = new_control_block;
}
...
inline WeakPtr<T>::WeakPtr() noexcept
    : control_block_(nullptr) {} // default constructor

template <class T>
inline WeakPtr<T[]>::WeakPtr() noexcept
    : control_block_(nullptr) {} // default constructor for arrays
...
inline WeakPtr<T>::WeakPtr(const WeakPtr<T> &other) noexcept // copy constructor
  ...
  if (control_block_ && control_block_->get() != nullptr)
    control_block_->increment_weak_ptr_reference_counter();
}

template <class T>
inline WeakPtr<T[]>::WeakPtr(
    const WeakPtr<T[]> &other) noexcept // copy constructor for arrays
    : control_block_(other.control_block_) {
  if (control_block_ && control_block_->get() != nullptr)
    control_block_->increment_weak_ptr_reference_counter();
}
...
inline WeakPtr<T[]>::WeakPtr(
    const SharedPtr<T[]>
        &other) noexcept // constructor from SharedPtr for arrays
    : control_block_(other.control_block_) {
  if (control_block_ && control_block_->get() != nullptr)
    control_block_->increment_weak_ptr_reference_counter();
}
...
template <class T>
inline WeakPtr<T[]>::WeakPtr(
    WeakPtr<T[]> &&other) noexcept // move constructor for arrays
    : control_block_(other.control_block_) {
  other.control_block_ = nullptr;
}
...
template <class T> inline WeakPtr<T[]>::~WeakPtr() {
...
template <class T> inline void WeakPtr<T>::reset() noexcept {
  update_reference(nullptr);
}
...
template <class T> inline bool WeakPtr<T>::expired() const noexcept {
  return use_count() == 0;
}
