...
inline ControlBlock<T>::ControlBlock(T *ptr) { // constructor from pointer
  if (!ptr)
    throw InvalidArgumentException("nullptr is not allowed");
...
inline ControlBlock<T[]>::ControlBlock(
    T *ptr) { // constructor from pointer for arrays
  if (!ptr)
    throw InvalidArgumentException("nullptr is not allowed");
...
inline void ControlBlock<T>::decrement_reference_counter_and_delete_if_zero() {
  if (reference_counter_ == 0)
    throw ZeroReferenceDecrementException();
...
template <class T>
inline void
ControlBlock<T[]>::decrement_reference_counter_and_delete_if_zero() {
  if (reference_counter_ == 0)
    throw ZeroReferenceDecrementException();
...
template <class T>
inline void
ControlBlock<T>::decrement_weak_ptr_reference_counter_and_delete_if_zero() {
  if (weak_ptr_reference_counter_ == 0)
    throw ZeroReferenceDecrementException();
...
template <class T> inline T *ControlBlock<T>::get() const noexcept {
  return ptr_;
}
...
template <class T>
inline unsigned int
ControlBlock<T>::get_weak_ptr_reference_counter() const noexcept {
